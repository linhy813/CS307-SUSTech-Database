...
try:
    ...
    createAll = PrepareStatement(db, SQLSentences.CREATE_ALL)
    createAll.cursor.execute(createAll.sql)
    createAll.con.commit()

    disableAll = PrepareStatement(db, SQLSentences.DISABLE_CONSTRAINT)
    disableAll.cursor.execute(disableAll.sql)
    disableAll.con.commit()
    ...
    # Stage1: individual: supply_center, product
    ...
    # Stage2：depend on company&product: salesman, product_model, company
    for row in df.itertuples(index=False):
        values = list(row)
        im.setValues(values)
    ...
    # Stage4：orders (depend on contracts, product_model, salesman)
    for row in df.itertuples(index=False):
        values = list(row)
        im.setValues(values)
